...
export default function Services() {
  ...
  async function remove(s) {
    ...
    try {
      ...
      // A service used by existing contracts cannot be removed without
      // rewriting those contracts, so offer the safe alternative instead.
      if (data?.canDeactivate && window.confirm(`${data.error}\n\nDeactivate it now?`)) {
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border/70">
                  {rows.map((s) => (
                    <tr key={s.id} className={cn('hover:bg-muted/30', s.is_active === false && 'opacity-55')}>
